...
async function handleRegister(event) {
    ...
    const { data, error } = await client.auth.signUp({
        ...
        password,
        options: {
            emailRedirectTo: publicConfig.emailRedirectTo || `${window.location.origin}/login.html`,
            data: {
                name
            }
        }
...
async function handleResetRequest(event) {
    ...
    const { error } = await client.auth.resetPasswordForEmail(email, {
        redirectTo: publicConfig.passwordResetRedirectTo || `${window.location.origin}/login.html?mode=reset`
    });
